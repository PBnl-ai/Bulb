...
export default function StatusIndicator({ isPlaying, isWeekday, isScheduledTime }: StatusIndicatorProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4" data-testid="status-indicator">
      <div className="flex items-center gap-4 p-6 rounded-2xl bg-white/80 backdrop-blur-sm border border-gray-200 shadow-sm">
        <div className={`p-3 rounded-xl ${isPlaying ? "bg-chart-3/20" : "bg-gray-100"}`}>
          <Radio className={`w-6 h-6 ${isPlaying ? "text-chart-3" : "text-gray-400"}`} />
        </div>
        <div>
      ...
      <div className="flex items-center gap-4 p-6 rounded-2xl bg-white/80 backdrop-blur-sm border border-gray-200 shadow-sm">
        <div className={`p-3 rounded-xl ${isWeekday ? "bg-primary/15" : "bg-gray-100"}`}>
          <Calendar className={`w-6 h-6 ${isWeekday ? "text-primary" : "text-gray-400"}`} />
        </div>
        <div>
      ...
      <div className="flex items-center gap-4 p-6 rounded-2xl bg-white/80 backdrop-blur-sm border border-gray-200 shadow-sm">
        <div className={`p-3 rounded-xl ${isScheduledTime ? "bg-primary/15" : "bg-gray-100"}`}>
          <Clock className={`w-6 h-6 ${isScheduledTime ? "text-primary" : "text-gray-400"}`} />
        </div>
        <div>
